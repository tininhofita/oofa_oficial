...
export function BarraLateral({
  ...
  const [submenusAbertos, definirSubmenusAbertos] = useState<
    Record<string, boolean>
  >({});

  // Efeito para abrir automaticamente o submenu correspondente à rota atual
  useEffect(() => {
    itensMenuFiltrados.forEach((item) => {
      if (
        item.subitens &&
        item.subitens.some((subitem) => rotaAtual.startsWith(subitem.link))
      ) {
  ...
  const itensMenu: ItemNavegacao[] = [
    ...
    },
    {
      ...
      subitens: [
        {
          titulo: "Importação",
          link: ROUTES.RELATORIOS_IMPORTACAO,
        },
      ],
    },
    {
  ...
  // Recupera permissões com fallback seguro
  const permissaoCargo = cargoUsuario && PERMISSIONS[cargoUsuario]
    ? PERMISSIONS[cargoUsuario]
    : {
  ...
  // Filtra os itens do menu de acordo com as permissões da role
  const itensMenuFiltrados = itensMenu.filter((item) => {
    if (item.link === ROUTES.USUARIOS) {
      return permissaoCargo.podeVerUsuarios;
    }
    if (item.link === ROUTES.FINANCEIRO) {
      return permissaoCargo.podeVerFinanceiro;
    }
    if (item.link === ROUTES.RELATORIOS) {
      return permissaoCargo.podeVerRelatorios;
    }
    if (item.link === ROUTES.INTEGRACOES) {
      return permissaoCargo.podeVerIntegracoes;
    }
    if (item.link === ROUTES.CONFIGURACOES) {
      return permissaoCargo.podeVerConfiguracoes;
    }
    return true;
  });
  ...
  return (
    <aside
      className={`barra-lateral ${barraLateralRecolhida ? "barra-lateral--recolhida" : ""}`}
    >
      <div className="barra-lateral__topo">
        <Link
          href={ROUTES.DASHBOARD}
          className="barra-lateral__logo-container"
      ...
      <nav className="barra-lateral__navegacao">
        {itensMenuFiltrados.map((item) => {
          ...
          const ativo =
            verificarAtivo(item.link) ||
            (item.subitens?.some((subitem) => verificarAtivo(subitem.link)) ??
              false);
              ...
              <div key={item.link} className="barra-lateral__item-grupo">
                ...
                {barraLateralRecolhida && (
                  <div className="barra-lateral__submenu-flutuante">
                    <div className="barra-lateral__submenu-titulo">
                      {item.titulo}
                    </div>
                    {item.subitens?.map((subitem) => {
                      const subAtivo = verificarAtivo(subitem.link);
      ...
      <div className="barra-lateral__rodape">
        <div className="barra-lateral__mini-perfil">
          ...
          <div className="barra-lateral__info-usuario">
            <span
              className="barra-lateral__nome-usuario"
              title={nomeUsuario || "Usuário"}
            >
              {nomeUsuario || "Usuário"}
            </span>
            <span
              className="barra-lateral__email-usuario"
              title={emailUsuario || "usuario@oofa.com"}
            >
